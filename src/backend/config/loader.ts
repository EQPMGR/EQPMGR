...
export function getBackendProviderName(): BackendProviderName {
  const provider = process.env.NEXT_PUBLIC_BACKEND_PROVIDER || process.env.BACKEND_PROVIDER || 'supabase';

  if (!(provider in CONFIG_LOADERS)) {
    console.warn(`Unknown backend provider: ${provider}. Defaulting to supabase.`);
    return 'supabase';
  }
