...
async function seedBottomBracketComponents() {
    ...
    if (count === 0) {
        return { success: true, message: "No new bottom brackets to seed." }
    }

    await batch.commit();

    return { success: true, message: `Successfully seeded ${count} bottom brackets.` };
}

export function BottomBracketSeeder() {
    ...
    const handleSeed = async () => {
        setIsSeeding(true);
        try {
            const result = await seedBottomBracketComponents();
            if (result.success) {
                toast({
    ...
    return (
        <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div>
                    <CardTitle className="text-base font-semibold">Seed Bottom Bracket Database</CardTitle>
                    <CardDescription className="text-sm">
                        Populate the database with common bottom brackets.
                    </CardDescription>
                </div>
