...
async function seedShimanoComponents() {
    ...
    if (count === 0) {
        return { success: true, message: "No new Shimano components to seed." }
    }

    await batch.commit();

    return { success: true, message: `Successfully seeded ${count} Shimano components.` };
}

export function ShimanoSeeder() {
    ...
    const handleSeed = async () => {
        setIsSeeding(true);
        try {
            const result = await seedShimanoComponents();
            if (result.success) {
                toast({
    ...
    return (
        <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div>
                    <CardTitle className="text-base font-semibold">Seed Shimano Database</CardTitle>
                    <CardDescription className="text-sm">
                        Populate the database with Shimano components.
                    </CardDescription>
                </div>
