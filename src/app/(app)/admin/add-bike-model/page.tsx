...
export type AddBikeModelFormValues = z.infer<typeof addBikeModelSchema> & {
  importFeedback?: {
    rawText?: string;
    parsedOutput?: ExtractBikeDetailsOutput;
    source?: string;
  };
};
...
async function getAvailableBrands(): Promise<string[]> {
    try {
        ...
        const querySnapshot = await database.getDocs("bikeModels",
          { type: 'orderBy', field: 'brand', direction: 'asc' },
...
function AddBikeModelFormComponent() {
    ...
    const [importFeedback, setImportFeedback] = useState<AddBikeModelFormValues['importFeedback'] | null>(null);
    ...
    const showValidationMessage = hasFormErrors && (form.formState.isSubmitted || form.formState.isDirty);
    ...
    useEffect(() => {
      try {
        ...
        if (storedData) {
          ...
          setImportFeedback({
            rawText: fullImportedData.rawText,
            parsedOutput: importedData,
            source: 'import-text',
          });
          ...
          importedData.components.forEach(importedComp => {
              const normalizedImportedComp = {
                ...importedComp,
                chainring1: importedComp.chainring1 || importedComp['chainring 1'] || importedComp['chainring-1'] || importedComp['Chainring 1'] || importedComp['Chainring-1'] || importedComp['chainring1'],
                chainring2: importedComp.chainring2 || importedComp['chainring 2'] || importedComp['chainring-2'] || importedComp['Chainring 2'] || importedComp['Chainring-2'] || importedComp['chainring2'],
                chainring3: importedComp.chainring3 || importedComp['chainring 3'] || importedComp['chainring-3'] || importedComp['Chainring 3'] || importedComp['Chainring-3'] || importedComp['chainring3'],
              };
              const cleanedName = normalizedImportedComp.name.toLowerCase().trim();
              
              const componentToUpdate = {
                  brand: normalizedImportedComp.brand || '',
                  series: normalizedImportedComp.series || '',
                  model: normalizedImportedComp.model || '',
                  size: normalizedImportedComp.size || '',
                  chainring1: normalizedImportedComp.chainring1 || '',
                  chainring2: normalizedImportedComp.chainring2 || '',
                  chainring3: normalizedImportedComp.chainring3 || '',
              };
              ...
              let wasMapped = false;
              const sideMatch = cleanedName.match(/\b(front|rear)\b/i);
              for (const key in dualComponentMap) {
                  if (cleanedName.includes(key)) {
                      dualComponentMap[key].forEach(targetName => {
                          ...
                          if (sideMatch) {
                              if (/\b(front|rear)\b/i.test(targetName)) {
                                  if (targetName.toLowerCase().startsWith(sideMatch[1].toLowerCase())) {
    ...
    async function onSubmit(values: AddBikeModelFormValues) {
        ...
        try {
            ...
            const payload = importFeedback ? { ...values, importFeedback } : values;
            const response = await fetch('/api/admin/add-bike-model', {
                method: 'POST',
                headers: {
        ...
        return (
            <Card>
                ...
                <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        const label = name === 'Cassette' && fieldName === 'size'
                            ? 'Teeth'
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl><Input placeholder={`Enter ${label.toLowerCase()}`} {...field} value={field.value || ''} /></FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
                        ...
                        <Accordion type="multiple" className="w-full space-y-4">
                            <AccordionItem value="frameset" className="border rounded-lg px-4">
                                ...
                                <AccordionContent className="space-y-4 pt-4">
                                    {renderComponentFields('Frame', ['brand', 'model'])}
                                    {renderComponentFields('Fork', ['brand', 'model'])}
                                    {renderComponentFields('Rear Shock', ['brand', 'model'])}
                                    {renderComponentFields('Headset', ['brand', 'model'])}
                                </AccordionContent>
                            </AccordionItem>
                            ...
                            <AccordionItem value="drivetrain" className="border rounded-lg px-4">
                                ...
                                <AccordionContent className="space-y-6 pt-4">
                                    ...
                                    {renderComponentFields('Bottom Bracket', ['brand', 'model'])}
                                    ...
                                    <Card><CardHeader><CardTitle className="text-lg">Shifters</CardTitle></CardHeader><CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Front Shifter</h4>
                                            ...
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Rear Shifter</h4>
                                                <div className="grid grid-cols-1 gap-4">
                            ...
                            <AccordionItem value="brakes" className="border rounded-lg px-4">
                                ...
                                <AccordionContent className="space-y-6 pt-4">
                                    ...
                                    <Card><CardHeader><CardTitle className="text-lg">Brake System</CardTitle></CardHeader><CardContent className="space-y-6">
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Front Brake</h4>
                                                <div className="grid grid-cols-1 gap-4">
                                                    <FormField name={`components.${getComponentIndex('Front Brake')}.brand`} render={({ field }) => (<FormItem><FormLabel>Caliper Brand</FormLabel><FormControl><Input placeholder="e.g., Shimano" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                    <FormField name={`components.${getComponentIndex('Front Brake')}.series`} render={({ field }) => (<FormItem><FormLabel>Caliper Series</FormLabel><FormControl><Input placeholder="e.g., Dura-Ace R9270" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                    <FormField name={`components.${getComponentIndex('Front Brake')}.model`} render={({ field }) => (<FormItem><FormLabel>Caliper Model</FormLabel><FormControl><Input placeholder="Optional" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                ...
                                                <div className="mt-6">
                                                    <h4 className="font-semibold text-sm mb-2">Rotor</h4>
                                                    <div className="grid grid-cols-1 gap-4">
                                                        <FormField name={`components.${getComponentIndex('Front Rotor')}.model`} render={({ field }) => (<FormItem><FormLabel>Rotor Model</FormLabel><FormControl><Input placeholder="e.g., RT-CL900" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                        <FormField name={`components.${getComponentIndex('Front Rotor')}.size`} render={({ field }) => (<FormItem><FormLabel>Rotor Size</FormLabel><FormControl><Input placeholder="e.g., 160mm" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                    </div>
                                                </div>
                                            </div>
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Rear Brake</h4>
                                                <div className="grid grid-cols-1 gap-4">
                                                    <FormField name={`components.${getComponentIndex('Rear Brake')}.brand`} render={({ field }) => (<FormItem><FormLabel>Caliper Brand</FormLabel><FormControl><Input placeholder="e.g., Shimano" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                    <FormField name={`components.${getComponentIndex('Rear Brake')}.series`} render={({ field }) => (<FormItem><FormLabel>Caliper Series</FormLabel><FormControl><Input placeholder="e.g., Dura-Ace R9270" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                    <FormField name={`components.${getComponentIndex('Rear Brake')}.model`} render={({ field }) => (<FormItem><FormLabel>Caliper Model</FormLabel><FormControl><Input placeholder="Optional" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                ...
                                                <div className="mt-6">
                                                    <h4 className="font-semibold text-sm mb-2">Rotor</h4>
                                                    <div className="grid grid-cols-1 gap-4">
                                                        <FormField name={`components.${getComponentIndex('Rear Rotor')}.model`} render={({ field }) => (<FormItem><FormLabel>Rotor Model</FormLabel><FormControl><Input placeholder="e.g., RT-CL800" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                        <FormField name={`components.${getComponentIndex('Rear Rotor')}.size`} render={({ field }) => (<FormItem><FormLabel>Rotor Size</FormLabel><FormControl><Input placeholder="e.g., 140mm" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <AccordionItem value="wheelset" className="border rounded-lg px-4">
                                ...
                                <AccordionContent className="space-y-6 pt-4">
                                    ...
                                    <Card><CardHeader><CardTitle className="text-lg">Rims</CardTitle></CardHeader><CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Front Rim</h4>
                                                <div className="grid grid-cols-1 gap-4">
                                                    ...
                                                    <FormField name={`components.${getComponentIndex('Front Rim')}.series`} render={({ field }) => (<FormItem><FormLabel>Series</FormLabel><FormControl><Input placeholder="e.g., 404 Firecrest" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                                            ...
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Rear Rim</h4>
                                                <div className="grid grid-cols-1 gap-4">
                                    ...
                                    <Card><CardHeader><CardTitle className="text-lg">Tires</CardTitle></CardHeader><CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Front Tire</h4>
                                                <div className="grid grid-cols-1 gap-4">
                                            ...
                                            <div>
                                                <h4 className="font-semibold text-sm mb-2">Rear Tire</h4>
                                                <div className="grid grid-cols-1 gap-4">
                            ...
                            <AccordionItem value="cockpit" className="border rounded-lg px-4">
                                ...
                                <AccordionContent className="space-y-4 pt-4">
                                    {renderComponentFields('Handlebar', ['brand', 'model'])}
                                    {renderComponentFields('Stem', ['brand', 'model'])}
                                    {renderComponentFields('Aero Bars', ['brand', 'model'])}
                                    {renderComponentFields('Seatpost', ['brand', 'model', 'size'])}
                                    {renderComponentFields('Saddle', ['brand', 'model'])}
                                    {renderComponentFields('Grips', ['brand', 'model'])}
                                    {renderComponentFields('Seatpost Clamp', ['brand', 'model', 'size'])}
                                </AccordionContent>
                            </AccordionItem>
                        ...
                        <div className="min-w-0">
                            {showValidationMessage && (
                                <p className="text-sm text-amber-600">
                                    Some required information is missing. Please complete the highlighted fields before saving.
                                </p>
                            )}
                        </div>
