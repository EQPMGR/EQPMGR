...
export default function InsurancePage() {
    ...
    const showBrokerInterestBanner = searchParams?.get('brokerInterest') === '1';

    useEffect(() => {
      const brokerInterest = searchParams?.get('brokerInterest');
      if (brokerInterest === '1') {
        form.setValue('contactWhenBrokerAvailable', true);
      }
    }, [searchParams, form]);
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            {showBrokerInterestBanner && (
              <div className="rounded-lg border border-yellow-300 bg-yellow-50 p-4 text-sm text-yellow-950">
                We could not confirm your province by GPS; we scored a non-BC result. Please confirm your details below and we will connect you with a broker when available.
              </div>
            )}
            <Form {...form}>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="mt-4 border-t pt-4">
                                <FormField
                                    control={form.control}
                                    name="contactWhenBrokerAvailable"
                                    render={({ field }) => (
                                        <FormItem className="flex items-start space-x-2">
                                            ...
                                            <div>
                                                <FormLabel className="font-normal">Notify me when a broker is available in my area</FormLabel>
                                                <p className="text-sm text-muted-foreground">We will contact you if we can arrange local coverage in your region.</p>
                                            </div>
                                        </FormItem>
                                    )}
                                />
