...
export default function EquipmentDetailPage() {
  ...
  const fetchEquipment = useCallback(async (uid: string) => {
    ...
    try {
      ...
      let allEquipmentSnapshot = await database.getDocs<Equipment>(
        'equipment',
        { type: 'where', field: 'userId', op: '==', value: uid }
      );

      if (allEquipmentSnapshot.empty) {
        allEquipmentSnapshot = await database.getDocs<Equipment>(
          'equipment',
      ...
      const equipmentDocSnap = await database.getDoc<Equipment>('equipment', equipmentId);
      ...
      if (equipmentDocSnap.exists) {
        ...
        const componentsSnapshot = await database.getDocs<UserComponent>(
          'components',
          { type: 'where', field: 'equipmentId', op: '==', value: equipmentId }
        );
  ...
  const handleUpdateEquipment = async (data: Partial<Equipment>) => {
    ...
    const database = await getDb();
    await database.updateDoc('equipment', equipment.id, data);
  ...
  const handleDeleteEquipment = async () => {
    ...
    try {
      ...
      const componentsSnap = await database.getDocs(
        'components',
        { type: 'where', field: 'equipmentId', op: '==', value: equipment.id }
      );
      componentsSnap.docs.forEach(doc => {
        batch.delete('components', doc.id);
      });

      // Delete the main equipment document
      batch.delete('equipment', equipment.id);
  ...
  const handleAddLog = async (newLog: Omit<MaintenanceLogType, 'id'>) => {
    ...
    const database = await getDb();
    await database.updateDoc('equipment', equipment.id, {
