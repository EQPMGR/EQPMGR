...
import { findExistingMasterComponent, createComponentSlug } from '@/lib/master-component-utils';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export function AddWheelsetDialog({
  ...
  const createComponentEntry = async (
      ...
      if (data.type === 'manual' && data.manualBrand && data.manualModel) {
          const componentToMatch = {
              name,
              brand: data.manualBrand,
              model: data.manualModel,
              size: data.manualModel,
              system,
          };
          const existingMaster = await findExistingMasterComponent(db, componentToMatch);
          if (existingMaster) {
              masterId = existingMaster.id;
          } else {
              masterId = db.generateId();
              const slug = createComponentSlug(componentToMatch as Partial<MasterComponent>);
              batch.set('masterComponents', masterId, {
                  name,
                  brand: data.manualBrand,
                  model: data.manualModel,
                  size: data.manualModel,
  ...
  const onSubmit = async (data: WheelsetFormValues) => {
    ...
    try {
      ...
      // Create components
      await createComponentEntry(batch, database, data.frontHub, 'Front Hub', 'Wheelset', wheelsetId);
      await createComponentEntry(batch, database, data.rearHub, 'Rear Hub', 'Wheelset', wheelsetId);
      await createComponentEntry(batch, database, data.frontRim, 'Front Rim', 'Wheelset', wheelsetId);
      await createComponentEntry(batch, database, data.rearRim, 'Rear Rim', 'Wheelset', wheelsetId);
      await createComponentEntry(batch, database, data.frontTire, 'Front Tire', 'Wheelset', wheelsetId);
      await createComponentEntry(batch, database, data.rearTire, 'Rear Tire', 'Wheelset', wheelsetId);
      await createComponentEntry(batch, database, data.cassette, 'Cassette', 'Drivetrain', wheelsetId);
