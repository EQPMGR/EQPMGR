...
export function EditEquipmentDialog({ equipment, allBikes, onUpdateEquipment, onOpenChange }: EditEquipmentDialogProps) {
  ...
  const form = useForm<EditEquipmentFormValues>({
    ...
    defaultValues: {
      type: 'bike',
      name: equipment.name || '',
      purchaseCondition: equipment.purchaseCondition || 'new',
      purchaseDate: equipment.purchaseDate || new Date(),
      purchasePrice: equipment.purchasePrice ?? 0,
      serialNumber: equipment.serialNumber || '',
      frameSize: equipment.frameSize || '',
      size: equipment.size ? parseFloat(equipment.size.split(' ')[0]) : 0,
      shoeSizeSystem: equipment.shoeSizeSystem || 'eu',
  ...
  async function onSubmit(data: EditEquipmentFormValues) {
    ...
    try {
        const normalizedPurchaseDate = data.purchaseDate instanceof Date
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g., My trusty steed" {...field} value={field.value ?? ''} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Input type="number" step="0.5" placeholder="e.g., 43.5" {...field} value={field.value ?? ''} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                    />
                ...
                <div className="grid grid-cols-2 gap-4">
                    <FormField
                        ...
                        <FormItem>
                            ...
                            <FormControl>
                                <Input placeholder="e.g., 56cm" {...field} value={field.value ?? ''} />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                        )}
